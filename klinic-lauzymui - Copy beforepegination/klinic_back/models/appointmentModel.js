...
exports.filterAppointments = async (filter) => {
  try {
    ...
    console.log("DB query result:", appointments);  
    return appointments;
  } catch (error) {
...
exports.createAppointmentRating = async (id, userId, visitData) => {
  ...
  // Jei userId yra pateiktas, filtruojame pagal jį, jei ne – adminas redaguoja viską
  const whereCondition = userId ? sql`id = ${id} AND user_id = ${userId}` : sql`id = ${id}`;
