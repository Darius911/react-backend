...
exports.createAppointment = async (req, res, next) => {
  ...
  exports.updateAppointment = async (req, res, next) => {
    try {
        ...
        // Jei `status` nėra siunčiamas iš frontend'o, nustatome 'pending'
        if (!newAppointmentData.status) {
...
exports.createAppointmentRating = async (req, res, next) => {
  try {
    ...
    if (req.user.role === "admin") {
      // Adminas gali atnaujinti bet kurį įrašą
      updatedVisit = await createAppointmentRating(id, null, req.body);
    } else {
      // Paprastas vartotojas gali atnaujinti tik savo įrašą
      updatedVisit = await createAppointmentRating(id, req.user.id, req.body);
    }

    if (!updatedVisit) {
      return next(new AppError("Visit not found or not allowed", 403));
    }
